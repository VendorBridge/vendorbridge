...
// Mini line chart (SVG)
// ─────────────────────────────────────────────────────
function SpendingChart({ data }: { data: { month: string; value: number }[] }) {
  if (!data || data.length === 0) return <div className="text-sm text-muted-foreground py-10">No spending data available.</div>;
  
  const maxVal = Math.max(...data.map((d) => d.value), 1000); // Ensure maxVal > 0
  ...
  return (
    ...
    >
      ...
      <path
        ...
        className={cn("transition-opacity duration-1000", animated ? "opacity-100" : "opacity-0")}
      />
      ...
      <path
        ...
        className={cn("transition-all duration-1000", animated ? "opacity-100" : "opacity-0")}
      />
      ...
      {points.map((p, i) => (
        <g key={i}>
          <circle
            ...
            strokeWidth="2"
            className={cn("transition-all duration-500", animated ? "opacity-100" : "opacity-0")}
...
// ─────────────────────────────────────────────────────
function CategoryDonut({ categories, totalSpend }: { categories: any[]; totalSpend: number }) {
  if (!categories || categories.length === 0 || totalSpend === 0) {
     return <div className="text-sm text-muted-foreground py-10">No spend by category data.</div>;
  }
    ...
    <div className="flex items-center gap-4">
      <svg viewBox="0 0 120 120" className="size-24 shrink-0">
        {categories.map((cat, i) => {
          const dashLen = (cat.value / totalSpend) * circumference;
      ...
      <div className="space-y-1.5">
        {categories.map((cat, i) => (
          <div key={cat.label} className="flex items-center gap-2 text-xs">
            ...
            />
            <span className="text-[hsl(var(--muted-foreground))] truncate max-w-[90px]">{cat.label.replace('_', ' ')}</span>
...
export default function DashboardClient({ 
  ...
  const STAT_CARDS: StatCard[] = [
    ...
    },
    {
      ...
      value: formatLakhs(stats.posThisMonthValue),
      change: `${stats.posThisMonthCount} orders`,
      trend: "up",
      icon: ShoppingCart,
      color: "hsl(160, 84%, 39%)",
                    ...
                    <td className="py-3.5 px-3">
                      <span
                        className={cn(
                          ...
                          STATUS_STYLES[po.status] || "bg-gray-500/15 text-gray-600 border-gray-500/20"
                        )}
                      >
                        {po.status}
                      </span>
